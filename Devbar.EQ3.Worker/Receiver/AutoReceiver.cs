    ...
    IBoolParser boolParser) : AbstractReceiver(configuration, thermostatManager, logger), IReceiver
{
    ...
    protected override string Command => "{0}/{1}/auto/command";
    protected override string Tag => "auto";
    protected override async Task Receive(object sender, BasicDeliverEventArgs args)
    {
        try {
            ...
            if (boolParser.Parse(body)) {
                status = await thermostat.SwitchToAutoModeAsync();
            } else {
                status = await thermostat.SwitchToManualModeAsync();
            }
